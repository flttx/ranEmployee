<template>
	<view class="content">
		...
		<p class="box-title">施工提成 <image src="../../../../static/imgs/common/add.png" class="click" @tap="clickAdd"></image></p>
		<view class="box">
			<view class="service-item" @tap="editMember" v-if="constructionMembers.length > 0" v-for="(item, index) in saleMembers" :key="index">
				<text class="service-item-name">{{item.name}}</text>
				<view class="service-item-price">{{item.commission}}元</view>
				<image class="delete-icon" src="../../../../static/imgs/common/delete.png" :data-index="index"  @tap.stop.prevent="deleteConstructionMember"></image>
			</view>
			<view class="service-item" v-else>
				<text class="no-data" @tap="clickAdd">当前无施工提成</text>
			</view>
		</view>
		
		<p class="box-title">销售提成 <image src="../../../../static/imgs/common/add.png" class="click" @tap="clickAdd"></image></p>
		<view class="box">
			<view class="service-item" @tap="editMember" v-if="saleMembers.length > 0" v-for="(item, index) in saleMembers" :key="index">
				<text class="service-item-name">{{item.name}}</text>
				<view class="service-item-price">{{item.commission}}元</view>
				<image class="delete-icon" src="../../../../static/imgs/common/delete.png" :data-index="index"  @tap.stop.prevent="deleteSaleMember"></image>
			</view>
			<view class="service-item" v-else>
				<text class="no-data" @tap="clickAdd">当前无销售提成</text>
			</view>
		</view>
